...
class LegacyApiController(Controller):
    ...
    @post("/query/", opt={"exclude_from_auth": False}, status_code=410)
    async def deprecated_query(self) -> Response[dict[str, str]]:
        """POST /api/v1/query/ -- deprecated query polling (was POST in Flask)."""
        return Response(
            content={"message": "Gone. Use /api/v1/sqllab/ instead."},
            status_code=410,
            headers={
    ...
    async def deprecated_form_data(self) -> Response[dict[str, str]]:
        """GET /api/v1/form_data/ -- deprecated form data."""
        return Response(
            content={
                "message": "Gone. Use /api/v1/explore/form_data/ instead."
            },
            status_code=410,
            headers={
    ...
    async def deprecated_time_range(self) -> Response[dict[str, str]]:
        """GET /api/v1/time_range/ -- deprecated time range."""
        return Response(
            content={
                "message": "Gone. Use chart data queries instead."
            },
            status_code=410,
            headers={
