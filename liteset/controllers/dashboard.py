...
from liteset.commands.dashboard import (
...
class DashboardController(Controller):
    ...
    async def create(
        ...
        event_logger.log("dashboard.create", object_ref=f"dashboard:{dashboard.id}", user_id=current_user.id)
        return DashboardGetResponse(
            id=dashboard.id,
            result={
                "dashboard_title": dashboard.dashboard_title,
    ...
    async def update(
        ...
        dashboard = await cmd.execute()
        event_logger.log("dashboard.update", object_ref=f"dashboard:{pk}", user_id=current_user.id)
        changed_on = getattr(dashboard, "changed_on", None)
        created_on = getattr(dashboard, "created_on", None)
        changed_by = getattr(dashboard, "changed_by", None)
        created_by = getattr(dashboard, "created_by", None)
    ...
    async def update_filters(
        ...
        dashboard = await cmd.execute()
        event_logger.log("dashboard.update_filters", object_ref=f"dashboard:{pk}", user_id=current_user.id)
    ...
    async def update_colors(
        ...
        await cmd.execute()
        event_logger.log("dashboard.update_colors", object_ref=f"dashboard:{pk}", user_id=current_user.id)
    ...
    async def delete_dashboard(
        ...
        await cmd.execute()
        event_logger.log("dashboard.delete", object_ref=f"dashboard:{pk}", user_id=current_user.id)
        return {"message": "OK"}
    ...
    async def bulk_delete(
        ...
        await cmd.execute()
        event_logger.log("dashboard.bulk_delete", user_id=current_user.id, extra={"count": len(ids)})
        return {"message": "OK"}
    ...
    async def export(
        self, dao: DashboardDAOProtocol, rison_params: dict[str, Any] | None
    ) -> Stream:
        ...
        return Stream(
            ...
            media_type="application/zip",
            headers={
                "Content-Disposition": "attachment; filename=dashboards_export.zip"
            },
        )
    ...
    async def add_favorite(
        ...
        event_logger.log("dashboard.add_favorite", object_ref=f"dashboard:{pk}", user_id=current_user.id)
        return {"message": "OK"}
    ...
    async def remove_favorite(
        ...
        event_logger.log("dashboard.remove_favorite", object_ref=f"dashboard:{pk}", user_id=current_user.id)
        return {"message": "OK"}
    ...
    async def create_embedded(
        ...
        event_logger.log("dashboard.create_embedded", object_ref=f"dashboard:{id_or_slug}")
        return {
            "result": EmbeddedDashboardResponse(
                uuid=str(embedded.uuid),
                allowed_domains=embedded.allowed_domains or [],
    ...
    async def update_embedded(
        ...
        embedded = await cmd.execute()
        event_logger.log("dashboard.update_embedded", object_ref=f"dashboard:{id_or_slug}")
        return {
            "result": EmbeddedDashboardResponse(
                uuid=str(embedded.uuid),
                allowed_domains=embedded.allowed_domains or [],
    ...
    async def delete_embedded(
        ...
        await cmd.execute()
        event_logger.log("dashboard.delete_embedded", object_ref=f"dashboard:{id_or_slug}")
        return {"message": "OK"}
    ...
    async def copy_dashboard(
        ...
        event_logger.log("dashboard.copy", object_ref=f"dashboard:{id_or_slug}", user_id=current_user.id)
        return {
            "result": {
                "id": new_dash.id,
