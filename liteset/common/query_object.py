...
try:
    ...
    class QueryObjectValidationError(Exception):  # type: ignore[no-redef]
        """Fallback when superset is not installed."""


try:
    from superset.utils.core import sanitize_clause
except ImportError:
    sanitize_clause = None  # type: ignore[assignment]
...
class AsyncQueryObject:
        ...
        for clause_key in ("where", "having"):
            ...
            if clause:
                try:
                    ...
                    sanitize_clause(clause, engine=engine)
    ...
    def _validate_time_offsets(self) -> None:
        ...
        for offset in self.time_offsets:
            ...
            # Check if offset is a date range and if the feature flag is enabled
            try:
                from superset.utils.date_parser import is_date_range

                if is_date_range(offset):
                    try:
