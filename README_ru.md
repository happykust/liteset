...
Схема таблиц метаданных (`ab_user`, `ab_role`, `dashboards`, `slices`, `tables`, `dbs`, `query`, `saved_query`, `report_schedule` и т.д.) наследуется от Apache Superset 6.0.0 — модели, имена и типы колонок не меняются, поэтому существующая база Superset читается и пишется Liteset'ом как есть.

А вот история Alembic перенесена **не** целиком: 339 ревизий апстрима схлопнуты в одну начальную (`c233f5365c9e`), поверх которой Liteset добавляет свои. Отсюда два следствия:

- База метаданных должна быть уже на head-ревизии апстрима `c233f5365c9e` (релиз 2025-08-05). Если она старее, сначала нужно прогнать цепочку апстрима до этой ревизии на legacy-инсталляции Apache Superset.
- После этого `alembic upgrade head` **обязателен** — собственные ревизии Liteset исправляют расхождения DDL. Это не подмена бекенда без миграций.

Точная процедура — в [`superset/migrations/README.md`](superset/migrations/README.md).
