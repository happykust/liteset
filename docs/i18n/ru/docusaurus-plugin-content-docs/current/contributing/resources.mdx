...
import InteractiveSVG from '@site/src/components/InteractiveERDSVG';
...
В диаграмме ниже подпись веб-сервера для Liteset следует читать как «Litestar / Uvicorn / SQLAlchemy 2.0 (async) / Pandas» (вместо «Flask / SQLAlchemy / Pandas»). Остальная топология — балансировщик, БД метаданных, слой кэшей, очередь, async-воркеры на Celery, внешние аналитические БД — идентична Apache Superset.
...
flowchart TD
  ...
  subgraph WebServers ["<b>Web Server(s)</b>"]
    ...
    WS2["<b>Backend</b><br/>(Python, Litestar, SQLAlchemy 2.0 async, Pandas, ...)"]
  end
...
Интерактивная ER-диаграмма (схема БД метаданных у Liteset идентична Apache Superset 6.0.0):

<InteractiveSVG />

<br />

[Скачать .svg](https://github.com/happykust/liteset/tree/main/docs/static/img/erd.svg)
