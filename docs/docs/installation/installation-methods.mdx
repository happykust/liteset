...
:::info
Liteset is a drop-in async replacement for Apache Superset 6.0.0 and reuses the same metadata-DB schema, frontend bundle, and Celery worker. Most installation patterns documented here are inherited from upstream Apache Superset and apply unchanged.
:::
