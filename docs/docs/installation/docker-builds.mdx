...
:::info Liteset images
This page documents the **Apache Superset** Docker build matrix and tagging conventions, since Liteset re-uses the same `Dockerfile` layout (lean / dev / py311 / ci targets, `BUILD_TRANSLATIONS`/`DEV_MODE`/`INCLUDE_CHROMIUM` ARGs, multi-arch builds). Liteset images are produced by the same workflow but pushed to a separate registry — `ghcr.io/happykust/liteset` (or your private build of the repo). For most users the practical difference is:

- **Apache Superset image** (`apache/superset:6.0.0`): runs Flask under Gunicorn.
- **Liteset image** (`ghcr.io/happykust/liteset:6.0.0`): runs Litestar under Uvicorn (`uvicorn superset.app:create_app --factory ...` as the container CMD).

If you build your own production image (recommended — see below), copy the example Dockerfile and replace the `FROM` line with the Liteset base image.
:::

The Apache Superset community extensively uses Docker for development, release, and productionising Superset. This page details the Docker builds and tag naming schemes inherited by Liteset.

Apache Superset images are built and pushed to the [Superset Docker Hub repository](https://hub.docker.com/r/apache/superset) using GitHub Actions.
Different sets of images are built and/or published at different times:

- **Published releases** (`release`): published using
  tags like `6.0.0` and the `latest` tag.
...
- `lean`: The default Docker image, including both frontend and backend. Tags
  without a build_preset are lean builds (ie: `latest`, `6.0.0`, `4.1.2`, ...). `lean`
...
- `websocket`: For Apache Superset clusters supporting advanced features. **Not used by Liteset** — WebSockets are served by Litestar in the main container.
...
subcommand and the [docker.yml](https://github.com/happykust/liteset/blob/main/.github/workflows/docker.yml)
GitHub action.
...
You would build the image with `docker build -t mysuperset:latest .` or `docker build -t ourcompanysuperset:6.0.0 .`

```Dockerfile
# change this to apache/superset:6.0.0 or whatever version you want to build from;
