...
with our [installing on Kubernetes](/docs/installation/kubernetes)
documentation.
:::

As mentioned in our [quickstart guide](/docs/quickstart), the fastest way to try
Liteset locally is using Docker Compose on a Linux or Mac OSX
computer. Liteset does not have official support for Windows. It's also the easiest
...
1. **docker-compose-image-tag.yml** where we fetch an image from docker-hub say for the
  `6.0.0` release for instance, and fire it up so you can try it. Here what's in
  ...
  `export TAG=6.0.0-dev` or `export TAG=5.0.0-dev`, with `latest-dev` being the default.
  All Liteset images ship `asyncpg` (used at runtime by the async ASGI app) and
  `psycopg2-binary` (used by Alembic migrations) — both are listed in
  `requirements/base.txt`, so no extra driver layering is needed to connect to the
  bundled Postgres metadata database.
...
## 1. Clone Liteset's GitHub repository

[Clone the Liteset repo](https://github.com/happykust/liteset) (or [the upstream Apache Superset repo](https://github.com/apache/superset) if you want to run vanilla Superset) in your terminal with the
following command:
...
Once that command completes successfully, you should see a new `liteset` folder in your
current directory.

## 2. Launch Liteset Through Docker Compose
...
like to try out Liteset without making any code changes follow the steps documented for
...
(ie: `git checkout 6.0.0`) for more guaranteed results. This ensures that the `docker-compose.*.yml`
...
:::caution
All of the content belonging to a Liteset instance - charts, dashboards, users, etc. - is stored in
...
this output slows, you should have a running instance of Liteset on your local machine! To avoid
the wall of text on future runs, add the `-d` option to the end of the `docker compose up` command.

### Configuring Further

The following is for users who want to configure how Liteset runs in Docker Compose; otherwise, you
can skip to the next section.

You can install additional python packages and apply config overrides by following the steps
mentioned in [docker/README.md](https://github.com/happykust/liteset/tree/main/docker#configuration)
...
are helpful for learning and testing out Liteset but unnecessary for experienced users and
...
:::note
Users often want to connect to other databases from Liteset. Currently, the easiest way to
...
`network_mode: host` on the Liteset services, but these generally break the installation,
...
Your local Liteset instance also includes a Postgres server to store your data and is already
pre-loaded with some example datasets that ship with Liteset. You can access Liteset now via your
...
## 4. Connecting Liteset to your local database instance

When running Liteset using `docker` or `docker compose` it runs in its own docker container, as if
Liteset was running in a separate machine entirely. Therefore attempts to connect to your local
database with the hostname `localhost` won't work as `localhost` refers to the docker container
Liteset is running in, and not your actual host machine. Fortunately, docker provides an easy way
...
Liteset (which is running in its docker container). Other databases may have slightly different
