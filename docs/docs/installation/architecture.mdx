...
1. The Liteset application itself (Litestar/ASGI on Uvicorn)
2. A metadata database (schema inherited 1:1 from Apache Superset 6.0.0)
3. A caching layer (optional, but necessary for some features)
4. A worker & beat (Celery, optional but necessary for some features)
...
Most people use Redis for their cache, but Liteset supports other options too. See the [cache docs](/docs/configuration/cache/) for more.
...
Other components can be incorporated into Liteset. The best place to learn about additional configurations is the [Configuration page](/docs/configuration/configuring-superset). For instance, you could set up a load balancer or reverse proxy to implement HTTPS in front of your Liteset application, or specify a Mapbox URL to enable geospatial charts, etc.
