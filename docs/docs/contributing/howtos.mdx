...
found here](https://github.com/happykust/liteset/tree/main/scripts/tests)
...
See [`superset-frontend/cypress_build.sh`](https://github.com/happykust/liteset/blob/main/superset-frontend/cypress_build.sh).
...
Debugging the Liteset Uvicorn app inside a Docker container (or attaching to a
Kubernetes pod) is covered in the development guide — see
[Debugging Server App](/docs/contributing/development#debugging-server-app)
and [Debugging Server App in Kubernetes Environment](/docs/contributing/development#debugging-server-app-in-kubernetes-environment).

The short version is: enable `SYS_PTRACE` on the container, install `debugpy`
inside, attach it to the parent Uvicorn PID, then point VSCode at port 5678.
...
Liteset ships a Flask-free, contextvars-based i18n layer in `superset/i18n.py`
that mirrors the Flask-Babel API the original code expected. Translation
extraction still uses `pybabel` and standard gettext `.po`/`.mo` files, but the
runtime helpers come from `superset.i18n`:
...
(pyproject.toml)[https://github.com/happykust/liteset/blob/main/pyproject.toml] file
