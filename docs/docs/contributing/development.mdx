...
This section covers the multiple ways to run Liteset locally for development (`docker compose`, plain `docker`, on "metal", using a Makefile).

:::note
The recommended path for most contributors is **`docker compose`** — it provisions a Liteset (Litestar/ASGI) backend, a Celery worker, a Postgres metadata DB, Redis and the frontend dev server in one shot. **Most people should stick to the first few sections — "Fork & Clone", "docker compose" and "Installing Dev Tools".**
:::

## Fork and Clone

First, [fork the repository on GitHub](https://help.github.com/articles/about-forks/), then clone your fork.
...
- this will pull/build docker images and run a cluster of services, including:
  - A Liteset **Litestar/Uvicorn web server** (replaces the Flask + Gunicorn process from Apache Superset), mounting the local Python repo/code
  - A **Celery worker**, also mounting the local Python repo/code (unchanged from upstream)
  - A **frontend dev server (Node)**, compiling and bundling the JS/TS assets
  ...
  - **Redis** as the message queue for Celery and the cache backend
- WebSockets are served **by Litestar itself** — no separate Node-based `superset-websocket` container is needed.
- Examples are loaded into the database on first startup.
- All other details and pointers are in [docker-compose.yml](https://github.com/happykust/liteset/blob/main/docker-compose.yml).
- The local repository is mounted within the services, so code edits on the host are reflected immediately (autoreload uses Uvicorn's `--reload`, not `flask run`).
- Liteset is served at `http://localhost:9000/`.
...
our [installing on k8s](/docs/installation/kubernetes) documentation.
...
[superset_config.py](https://github.com/happykust/liteset/blob/main/docker/pythonpath_dev/superset_config.py)
...
contributing to Liteset more accessible to developers worldwide.
:::
...
1. **Create a Codespace**: Use this pre-configured link that sets up everything you need:
   ...
   [**Launch Liteset Codespace →**](https://codespaces.new/happykust/liteset?quickstart=1)
...
- **Read [LLMS.md](https://github.com/happykust/liteset/blob/main/LLMS.md) first** - Contains comprehensive development guidelines, coding standards, and critical refactor information
...
For detailed development context, environment setup, and coding guidelines, see [LLMS.md](https://github.com/happykust/liteset/blob/main/LLMS.md).
...
Ensure that you are using a supported Python version (3.10 or 3.11), then proceed with:
...
# Install Liteset in editable (development) mode
pip install -e .

# Initialize the database (Alembic runs via psycopg2; runtime uses asyncpg)
superset db upgrade
...
# Start the Uvicorn dev server from inside your virtualenv.
# Note that your page may not have CSS at this point.
# See instructions below on how to build the front-end assets.
uvicorn superset.app:create_app --factory --reload --reload-dir superset --port 8088 --loop uvloop
```

The CLI also exposes a thin wrapper around Uvicorn that uses the same factory:

```bash
superset runserver --reload --port 8088
```
...
# Install Liteset pip packages and setup env only
$ make superset
...
Liteset is an ASGI app — there is no `FLASK_APP` env var and no `.flaskenv` file. The
application factory always lives at `superset.app:create_app` and is invoked by Uvicorn
with `--factory`. The legacy `--with-threads` / `--debugger` Flask flags do not apply
either; reloading is handled by Uvicorn's `--reload` and structured logging by Liteset's
`LOGGING_CONFIGURATOR_CLASS`.
...
Liteset runs on Litestar/Uvicorn, not WSGI, so the legacy `ConsoleLog`/`gunicorn -k geventwebsocket`
recipe no longer applies. Instead, configure logging via the `LOGGING_CONFIGURATOR_CLASS`
setting in `superset_config.py` — the configurator receives the Litestar `app` instance
on startup and can install custom handlers/filters. The defaults already silence noisy
`uvicorn.access` / `uvicorn.error` records (see `superset/logging.py`). The legacy
`FLASK_APP_MUTATOR` hook is still honoured for backwards compatibility and now receives
the Litestar app object.

### Frontend

Frontend assets (TypeScript, JavaScript, CSS, and images) must be compiled in order to properly display the web UI. The `superset-frontend` directory contains all NPM-managed frontend assets. Note that for some legacy pages there are additional frontend assets bundled with Flask-AppBuilder (e.g. jQuery and bootstrap); Liteset still ships these under `superset/static/appbuilder/` for FAB template compatibility. They are not managed by NPM and may be phased out in the future.
...
The inherited frontend still carries the upstream `@scarf/scarf` dependency in `package.json`, but **Liteset does not operate any Scarf telemetry gateway or analytics pixel** — see the [telemetry FAQ](/docs/faq#does-liteset-collect-any-telemetry-data). If you want to be extra sure the `scarf-js` npm package stays inert during local installs, set the `SCARF_ANALYTICS` environment variable to `false`, or disable it in `superset-frontend/package.json`:
...
1. [run Liteset locally](#uvicorn-dev-server) using Uvicorn, on port `8088` — but don't access it directly,<br/>

   ```bash
   # Install Liteset and dependencies, plus load your virtual environment first, as detailed above.
   uvicorn superset.app:create_app --factory --reload --reload-dir superset --port 8088 --loop uvloop
   ```
...
3. access `http://localhost:9000` (the Webpack server, _not_ the backend) in your web browser. This will use the hot-reloading front-end assets from the Webpack development server while redirecting back-end queries to Liteset: your changes on the codebase — either front or back-end — will then be reflected live in the browser.
...
# Proxy backend requests to a Liteset server running on a non-default port
...
The `--superset=` option is useful in case you want to debug a production issue or have to setup Liteset behind a firewall. It allows you to run the backend in another environment while keep assets building locally for the best developer experience.
...
If you want to use the same flag in the client code, also add it to the FeatureFlag TypeScript enum in [@superset-ui/core](https://github.com/happykust/liteset/blob/main/superset-frontend/packages/superset-ui-core/src/utils/featureFlags.ts). For example,
...
There is also a utility script included in the Superset codebase to run Python integration tests. The [readme can be found here](https://github.com/happykust/liteset/tree/main/scripts/tests).
...
Follow these instructions to debug the Liteset Uvicorn app running inside a docker
container. Note that this will run a barebones Liteset web server.
...
Find the PID for the Uvicorn parent process. Use the first (parent) PID. Uvicorn
spawns a new worker every time you change any python file when `--reload` is enabled,
so attaching to the parent keeps your breakpoints alive across reloads.
...
root         6     1  4 14:09 ?        00:00:04 /app/.venv/bin/python /app/.venv/bin/uvicorn superset.app:create_app --factory --host 0.0.0.0 --port 8088 --reload --reload-dir /app/superset --loop uvloop
root        10     6  7 14:09 ?        00:00:07 /app/.venv/bin/python /app/.venv/bin/uvicorn superset.app:create_app --factory --host 0.0.0.0 --port 8088 --reload --reload-dir /app/superset --loop uvloop
```

Inject debugpy into the running Uvicorn parent process. In this case PID 6.
...
Once the pod is running as root and has the SYS_PTRACE capability it will be able to debug the Uvicorn app.
