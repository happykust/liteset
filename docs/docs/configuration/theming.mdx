...
Liteset rides on **Ant Design v5's token-based theming**.
Every Antd token works, plus a handful of Liteset-specific ones for charts and dashboard chrome.

## Managing Themes via UI

Liteset includes a built-in **Theme Management** interface accessible from the admin menu under **Settings > Themes**.

### Creating a New Theme

1. Navigate to **Settings > Themes** in the Liteset interface
...
4. Paste the JSON into the theme definition field in Liteset
5. Give your theme a descriptive name and save

You can also extend with Liteset-specific tokens (documented in the default theme object) before you import.
...
- **Automatic OS Detection**: When both default and dark themes are set, Liteset automatically detects and applies the appropriate theme based on OS preferences
...
3. Administrators can change system themes without restarting Liteset
...
3. Use this JSON in your `superset_config.py` or import it into another Liteset instance

## Theme Development Workflow

1. **Design**: Use the [Ant Design Theme Editor](https://ant.design/theme-editor) to iterate on your design
2. **Test**: Create themes in Liteset's CRUD interface for testing
...
Liteset supports custom fonts through runtime configuration, allowing you to use branded or custom typefaces without rebuilding the application.
...
- **JSON Editor**: Edit theme configurations directly within Liteset's interface
...
For programmatic theme management, Liteset provides REST endpoints:
