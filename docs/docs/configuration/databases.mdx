...
Liteset does not ship bundled with connectivity to databases. The main step in connecting
Liteset to a database is to **install the proper database driver(s)**
in your environment.

:::note
You’ll need to install the required packages for the database you want to use as your metadata database
as well as the packages needed to connect to the databases you want to access through Liteset.
For information about setting up Liteset's metadata database, please refer to
...
Liteset requires a Python [DB-API database driver](https://peps.python.org/pep-0249/)
...
install new database drivers into your Liteset configuration.

### How Liteset connects: async and sync drivers

Liteset talks to your databases over SQLAlchemy 2.0's **async** engine. Unlike Apache Superset — which is synchronous end to end — Liteset has two connection paths, and it picks the right one automatically based on the database engine:

- **Native async drivers.** For engines that have a real asyncio DB-API, Liteset ships a dedicated async engine spec (`superset/db/engine_specs/`) that runs queries **without blocking the event loop**. While one query waits on the database, the same worker keeps serving other requests — this is where the throughput and tail-latency gains in the [benchmarks](/docs/benchmarks/results) come from.
- **Sync fallback.** Every other SQLAlchemy-supported database keeps working through `SyncFallbackEngineSpec`, which drives the **standard synchronous DB-API** (`psycopg2`, `pymysql`, `snowflake-sqlalchemy`, …) through SQLAlchemy's greenlet `run_sync` bridge. This preserves Superset's full database catalogue with no loss of functionality — the trade-off is that such a query occupies its coroutine until it returns, so it does not yield the event loop the way a native async driver does.

**You install the same driver packages and use the same SQLAlchemy URIs as in Apache Superset** (see the [table below](#supported-databases-and-dependencies)). You don't rewrite connection strings: Liteset resolves the engine spec from the URI backend and, when a native async spec and its driver are present, uses the async driver under the hood.

Engines with a **native async engine spec**:

| Database engine(s)                                                                                          | Native async driver       |
| ----------------------------------------------------------------------------------------------------------- | ------------------------- |
| PostgreSQL — and the Postgres-wire engines TimescaleDB, Hologres, YugabyteDB, AWS Redshift, CockroachDB, CrateDB, Denodo, RisingWave | `asyncpg`                 |
| MySQL, StarRocks                                                                                            | `asyncmy`                 |
| Apache Doris, OceanBase                                                                                     | `aiomysql`                |
| ClickHouse                                                                                                  | `asynch`                  |
| Trino                                                                                                       | `aiotrino`                |
| Microsoft SQL Server / Azure SQL                                                                            | `aioodbc`                 |
| Oracle                                                                                                      | `oracledb` (thin, async)  |
| Apache Pinot                                                                                                | async HTTP (`pinotdb`)    |
| AWS Athena                                                                                                  | `pyathena` (REST)         |
| AWS DynamoDB                                                                                                | `aioboto3`                |
| Elasticsearch / OpenDistro                                                                                  | async Elasticsearch client |
| Azure Data Explorer (Kusto — SQL & KQL)                                                                     | async HTTP                |
| Databend                                                                                                    | `databend`                |
...
:::note
The async drivers **bundled in the Liteset image** are `asyncpg`, `aiosqlite` (SQLite) and `aiotrino` — these cover the metadata database and the most exercised analytical engines. **PostgreSQL, MySQL, ClickHouse and Trino** are the four engines validated under the load tests. For any other engine — async or sync — install the matching driver package exactly as you would on Apache Superset, following the table in the next section.
:::

### Supported Databases and Dependencies

Some of the recommended packages are shown below. Please refer to
[pyproject.toml](https://github.com/happykust/liteset/blob/main/pyproject.toml) for the versions that
are compatible with Liteset.
...
[Liteset GitHub repo](https://github.com/happykust/liteset/issues), so we can work on documenting and
supporting it.

If you'd like to build a database connector for Liteset integration,
read the [following tutorial](https://preset.io/blog/building-database-connector/) (originally
written for Apache Superset — the connector contract is unchanged).

### Installing Drivers in Docker Images

Liteset requires a Python database driver to be installed for each additional
...
We need to get the `mysqlclient` library installed into the Liteset docker container
...
To address this, the Liteset `docker compose` deployment uses the convention
...
**If you are running a stock (non-customized) Liteset image**, you are done.
Launch Liteset with `docker compose -f docker-compose-non-dev.yml up` and
...
After the rebuild of the Docker images is complete, relaunch Liteset by
running `docker compose up`.

#### 3. Connect to MySQL

Now that you've got a MySQL driver installed in your container, you should be able to connect
to your database via the Liteset web UI.
...
Consult the page for your specific database type in the Liteset documentation to determine
...
If the test fails, review your docker logs for error messages.  Liteset uses SQLAlchemy
to connect to databases; to troubleshoot the connection string for your database, you might
start Python in the Liteset application container or host environment and try to connect
directly to the desired database and fetch data. This eliminates Liteset for the
purposes of isolating the problem.

Repeat this process for each type of database you want Liteset to connect to.
...
The PyAthena library also allows to assume a specific IAM role which you can define by adding following parameters in Liteset's Athena database connection UI under ADVANCED --> Other --> ENGINE PARAMETERS.
...
You have to define the following arguments in Liteset's redshift database connection UI under ADVANCED --> Others --> ENGINE PARAMETERS.
...
To use ClickHouse with Liteset, you will need to install the `clickhouse-connect` Python library:

If running Liteset using Docker Compose, add the following to your `./docker/requirements-local.txt` file:
...
To connect to a Hive cluster add a database of type "Databricks Interactive Cluster" in Liteset, and use the following SQLAlchemy URI:
...
When adding a new BigQuery connection in Liteset, you'll need to add the GCP Service Account
...
2. In Liteset, you can either upload that JSON or add the JSON blob in the following format (this should be the content of your credential JSON file):
...
To be able to upload CSV or Excel files to BigQuery in Liteset, you'll need to also add the
[pandas_gbq](https://github.com/pydata/pandas-gbq) library.

Liteset runs Uvicorn (asyncio + uvloop) instead of gunicorn — there is no monkey-patching of the
Python standard library on the hot path. The historical Apache Superset caveat about the Google
BigQuery Python SDK being incompatible with `gevent` workers therefore does **not** apply to
Liteset; connection pooling, retries, and the `google.cloud` SDKs all work natively under asyncio.
...
comes out of the box with Liteset (it is also required for Alembic metastore migrations).
...
If you're using docker compose, psycopg2 comes out of the box with Liteset.
...
`ALLOWED_EXTRA_AUTHENTICATIONS` allow list in the Liteset config file:
...
comes out of the box with Liteset.
...
For copyright reasons the logos for each database are not distributed with Liteset.
...
Liteset can be configured to use an external store for database passwords. This is useful if you are
running a custom secret distribution framework and do not wish to store secrets in Liteset’s meta
database.
