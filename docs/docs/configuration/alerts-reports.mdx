...
Note: All the components required (Firefox headless browser, Redis, Postgres db, celery worker and celery beat) are present in the *dev* docker image if you are following [Installing Liteset Locally](/docs/installation/docker-compose/).
All you need to do is add the required config variables described in this guide (See `Detailed Config`).

If you are running a non-dev Liteset image (the `liteset:lean` target or a tagged release), that image does not include a headless browser. Only the `superset_worker` container needs this headless browser to browse to the target chart or dashboard.
You can either install and configure the headless browser - see "Custom Dockerfile" section below - or when deploying via `docker compose`, modify your `docker-compose.yml` file to use a dev image for the worker container and a lean release image for the `superset_app` container.

*Note*: In this context, a "dev image" is the same application software as its corresponding lean image, just bundled with additional tools (Firefox, geckodriver, build deps).  An image like `liteset:dev` is identical to `liteset:lean` when it comes to stability, functionality, and running in production.
...
5. Select a default channel for your app and continue.
   (You can post to any channel by inviting your Liteset app into that channel).
...
- You must have a `celery beat` pod running. If you're using the chart included in the GitHub repository under [helm/superset](https://github.com/happykust/liteset/tree/main/helm/superset), you need to put `supersetCeleryBeat.enabled = true` in your values override.
...
This process also works in a Docker swarm environment, you would just need to add `Deploy:` to the Liteset, Redis and Postgres services along with your specific configs for your swarm.
...
You can find documentation about each field in the default `config.py` in the GitHub repository under [superset/config.py](https://github.com/happykust/liteset/blob/main/superset/config.py).

You need to replace default values with your custom Redis, Slack and/or SMTP config.

Liteset uses Celery beat and Celery worker(s) to send alerts and reports.
...
EMAIL_REPORTS_SUBJECT_PREFIX = "[Liteset] " # optional - overwrites default value in config.py of "[Report] "
...
- Adjust `WEBDRIVER_BASEURL` in your configuration file if celery workers can’t access Liteset via
...
If you're running the dev variant of a released Liteset image (`liteset:dev` or a tagged equivalent), you should be set with the above — Firefox and geckodriver are already baked in.

But if you're building your own image, or starting with the lean variant, a webdriver (and headless browser) is needed to capture screenshots of the charts and dashboards which are then sent to the recipient.
Here's how you can modify your Dockerfile to take the screenshots either with Firefox or Chrome. Liteset publishes its own images under the `liteset` namespace — substitute your own registry path if you self-host.
...
If you don't see "Alerts & Reports" under the *Manage* section of the Settings dropdown in the Liteset UI, you need to enable the `ALERT_REPORTS` feature flag (see above). Enable another feature flag and check to see that it took effect, to verify that your config file is getting loaded.
...
Liteset docker images that have a tag ending with `-dev` have the Firefox headless browser and geckodriver already installed. You can test that these are installed and in the proper path by entering your Liteset worker and running `firefox --headless` and then `geckodriver`. Both commands should start those applications.
...
Confirm via testing that your outbound email configuration is correct.  Here is the simplest test, for an un-authenticated email SMTP email service running on port 25.  If you are sending over SSL, for instance, study how [Liteset's codebase sends emails](https://github.com/happykust/liteset/blob/main/superset/reports/notifications/email.py) and then test with those commands and arguments.
