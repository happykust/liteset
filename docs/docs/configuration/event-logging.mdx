...
Liteset by default logs special action events in its internal database (DBEventLogger). These logs can be accessed
...
Liteset can be configured to log events to [StatsD](https://github.com/statsd/statsd)
...
If not already present, you need to ensure that the `statsd` package is installed in Liteset's Python environment.
