...
Authentication and authorization in Apache Superset are handled by Flask AppBuilder (FAB), an application development framework built on top of Flask. FAB provides authentication, user management, permissions and roles. Please read its [Security documentation](https://flask-appbuilder.readthedocs.io/en/latest/security.html) — most concepts in this page reference FAB primitives.

:::info Liteset implementation
Liteset ports the FAB security surface to an `AsyncSecurityManager` running on Litestar/ASGI. The configuration surface is **identical** to Apache Superset:

- The same `AUTH_TYPE`, `AUTH_USER_REGISTRATION`, `AUTH_ROLES_*`, `OAUTH_PROVIDERS`, `LDAP_*` keys in `superset_config.py`.
- The same `ab_user`, `ab_role`, `ab_permission_view`, `ab_user_role` tables in the metadata DB (no schema changes).
- The same Flask-signed session cookie format (decoded natively in Liteset) and the same `X-CSRFToken` header — sessions survive a migration.
- `SupersetSecurityManager` subclasses you wrote against Apache Superset still work, **but methods that are async in Liteset (`oauth_user_info`, `add_user`, `update_user_auth_stat`, `find_user`, …) must be `async def` overrides** when you derive from Liteset's `AsyncSecurityManager`. If you need to use existing sync code from a third-party library, wrap it with `await asyncio.to_thread(...)`.

When this page references `flask_appbuilder.security.manager.AUTH_OAUTH` or similar, the same constants are re-exported from `superset.security` for convenience and the Apache Superset documentation links remain authoritative for behavioural details.
:::
...
A table with the permissions for these roles can be found at [/RESOURCES/STANDARD_ROLES.md](https://github.com/happykust/liteset/blob/main/RESOURCES/STANDARD_ROLES.md).
...
Liteset reads and writes the same itsdangerous-signed session cookie that
Flask/Flask-Login produced in Apache Superset, so existing browser sessions
survive the migration without forcing users to re-login. Decoding happens
natively in Liteset middleware — there is no Flask-Login runtime in the
process.
...
The session cookie is signed with the application `SECRET_KEY`, so it's very
important to keep the `SECRET_KEY` secret and set to a secure unique complex
random value (see `LITESET_SECRET_KEY`).

Cookie behaviour is controlled by the same configuration keys upstream
Superset used — Liteset honours them via `SupersetSettings`:

`SESSION_COOKIE_HTTPONLY`: (default: `True`): Controls if cookies should be set with the `HttpOnly` flag.
...
The Apache Superset story around server-side sessions used Flask-Session.
Liteset does not depend on Flask-Session at runtime; the existing
`SESSION_SERVER_SIDE` / `SESSION_TYPE` / `SESSION_REDIS` configuration keys
are read by `SupersetSettings` and resolved into the native session backend
(Redis-backed when configured). The configuration surface stays compatible:

```python
SESSION_SERVER_SIDE = True
SESSION_TYPE = "redis"
SESSION_REDIS = "redis://redis:6379/0"  # URL string, not a Redis instance
SESSION_USE_SIGNER = True
```

Note: pass a URL string for `SESSION_REDIS` rather than a `Redis()` client
instance — Liteset constructs the redis-asyncio client itself.
...
TALISMAN_CONFIG = {
    "content_security_policy": {
        ...
        "img-src": [
            ...
            "blob:",
            "data:",
            # The two scarf.sh entries below are inherited from the upstream default CSP.
            # Liteset operates no Scarf telemetry pixel, so you can safely drop them.
...
If you discover a security vulnerability or potential threat in Liteset,
please report it privately rather than opening a public issue. Use GitHub's
private security advisory flow on the
[Liteset repository](https://github.com/happykust/liteset/security/advisories/new),
which lets you describe the issue, include a proof-of-concept, and coordinate
a fix and disclosure timeline with the maintainers.

Because Liteset preserves Apache Superset's behavioural surface, vulnerabilities
that affect upstream Apache Superset may also affect Liteset. For those, the
Apache Software Foundation's security process at
[apache.org/security](https://apache.org/security/committers.html) remains
authoritative; Liteset will track and mitigate published Apache Superset CVEs
where the affected code path also exists in this fork — see the
[Superset CVEs](/docs/security/cves) page.

Please do not disclose vulnerabilities on the public issue tracker, in chat
rooms, or in pull requests before coordination with the maintainers.
