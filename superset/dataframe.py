...
Used by ``sql_lab``, ``viz`` and ``views/utils`` to convert in-memory
result frames into plain JSON records while preserving the JS-safe
big-integer cast that the front-end relies on (any integer larger than
``JS_MAX_INTEGER`` loses precision once it crosses the JSON boundary
into the browser).
"""
