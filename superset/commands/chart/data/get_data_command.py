...
from superset.exceptions import CacheLoadError, CommandInvalidError, ForbiddenError
...
class GetCachedChartDataCommand(AsyncBaseCommand[dict[str, Any] | None]):
    ...
    LITESET ADDITION (no 1:1 counterpart in ``superset_old/commands/``).
    The original Apache Superset reads cached chart data inline inside
    ``superset_old/charts/data/api.py::data_from_cache``; this command
    encapsulates that flow so the async controller stays thin.
    ...
    async def _raise_for_access(self, raw: dict[str, Any]) -> None:  # noqa: C901
        ...
        - ``datasource_id`` / ``datasource_type`` fields written by superset.
