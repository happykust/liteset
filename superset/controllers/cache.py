...
class DatasourceRef(msgspec.Struct, kw_only=True):
    ...
    NB: NO ``rename="camel"`` — the wire fields are snake_case
    (``database_name``/``datasource_name``/``datasource_type``), which is also
    what the published OpenAPI spec documents. Camel-renaming would silently
    drop the documented payload.
    ...
    # ``schema`` has no allow_none — an explicit null must be rejected (→ 400),
    # only absence is allowed; ``catalog`` above IS allow_none=True.
...
class CacheInvalidateSchema(msgspec.Struct):
    ...
    Accepts either direct UIDs or datasource name tuples (or both). Wire fields
    are snake_case (``datasource_uids``/``datasources``); do NOT add
    ``rename="camel"`` — the OpenAPI spec uses snake_case, and camel-renaming
    silently drops a correctly-formed ``datasource_uids`` body (→ 201 invalidating
    nothing).
    """
...
class CacheController(Controller):
    ...
    async def invalidate(
        ...
        database records.
        """
        # Use the original class name ``CacheRestApi`` for metric key
    ...
    async def _do_invalidate(
        ...
        do NOT use a finally block here; only log on success.
        """
        ...
        # ``object_ref`` must be set explicitly: without it _alog_with_context's
        # ``if object_ref:`` guard is False and the field is absent from logs.json.
    ...
    async def _invalidate_body(
        ...
        try:
            ...
            # response_500 returns {"message": str(ex)} — pass the actual
            # exception text, not a fixed string, to match the client-visible body.
