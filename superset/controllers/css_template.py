...
class CssTemplateController(Controller):
    ...
    async def related(
        ...
        """Applies ``BaseFilterRelatedUsers`` on the ``changed_by`` field, excluding
        users in ``EXCLUDE_USERS_FROM_LISTS`` and applying the
        ``EXTRA_RELATED_QUERY_FILTERS["user"]`` hook.
        """
        ...
        if column_name == "changed_by":
            ...
            # EXTRA_RELATED_QUERY_FILTERS["user"] hook — Callable[[Query], Query];
            # passed as query_hook so get_related_payload applies it to the SA 2.0
            # Select statement.  No try/except — any exception propagates as HTTP 500.
            ...
            # When EXCLUDE_USERS_FROM_LISTS is None, fall back to
            # security_manager.get_exclude_users_from_lists() — original behaviour.
        ...
        # Text filter is suppressed for 'created_by' because it has no entry in
        # related_field_filters (only 'changed_by' has a text filter).
