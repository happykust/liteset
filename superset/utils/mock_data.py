...
# SQLAlchemy 2.0 renamed ``VisitableType`` to ``Visitable``.  The
# original Superset module imports ``VisitableType`` from 1.4.
from sqlalchemy.sql.visitors import Visitable as VisitableType
...
def add_data(
    ...
    with _ctx.example_engine(database) as engine:
        table_exists = _ctx.has_table(engine, table_name)
        ...
        if columns is None:
            ...
            columns = cast("list[ColumnInfo]", inspector.get_columns(table_name))
...
def add_sample_rows(model: type[Base], count: int) -> Iterator[Base]:
    """
    Add entities of a given model.

    :param Model model: a Superset/SQLAlchemy declarative model
    ...
    samples = _ctx.session.query(model).limit(count).all() if relationships else []
