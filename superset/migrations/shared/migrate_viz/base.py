...
def _default_time_filter() -> Any:
    ...
    setting lives on :class:`SupersetSettings`; we instantiate a fresh
    one via the standard Pydantic factory because the migration code
    never enters the request loop and we don't want to depend on
    ``current_app``.
    """
    try:
        from superset.config import SupersetSettings

        return SupersetSettings().default_time_filter  # type: ignore[call-arg]
...
class MigrateViz:
    ...
    def _migrate_temporal_filter(self, rv_data: dict[str, Any]) -> None:
        ...
        time_range = rv_data.pop("time_range", None) or _default_time_filter()
