    ...
    with _ctx.example_engine(database) as engine:
        ...
        pdf.to_sql(
            ...
            dtype={
                ...
                "ds": DateTime if backend != "presto" else String(255),
                "gender": String(16),
                "state": String(10),
                "name": String(255),
            },
...
def _add_table_metrics(datasource: SqlaTable) -> None:
    ...
    if not any(col.column_name == "num_california" for col in columns):
        col_state = str(column("state").compile(_ctx.engine))
        col_num = str(column("num").compile(_ctx.engine))
        columns.append(
            TableColumn(
...
def create_dashboard(slices: list[Slice]) -> Dashboard:
    ...
    dash = _ctx.session.query(Dashboard).filter_by(slug="births").first()
    if not dash:
        dash = Dashboard()
