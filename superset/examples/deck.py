...
def load_deck_dash() -> None:  # pylint: disable=too-many-statements
    ...
    tbl = _ctx.session.query(table).filter_by(table_name="long_lat").first()
    slice_data = {
    ...
    polygon_tbl = (
        _ctx.session.query(table).filter_by(table_name="sf_population_polygons").first()
    )
    slice_data = {
    ...
    slc = Slice(
        ...
        viz_type="deck_arc",
        datasource_type=DatasourceType.TABLE,
        datasource_id=_ctx.session.query(table)
    ...
    title = "deck.gl Demo"
    dash = _ctx.session.query(Dashboard).filter_by(slug=slug).first()
