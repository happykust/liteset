...
def load_world_bank_health_n_pop(  # pylint: disable=too-many-locals
        ...
        if not only_metadata and (not table_exists or force):
            ...
            pdf.to_sql(
                ...
                dtype={
                    ...
                    "year": DateTime if backend != "presto" else String(255),
    ...
    description_path = os.path.join(get_examples_folder(), "countries.md")
    with open(description_path, encoding="utf-8") as f:
        tbl.description = f.read()
    ...
    for metric in metrics:
        if not any(col.metric_name == metric for col in tbl.metrics):
            aggr_func = metric[:3]
            col = str(column(metric[5:]).compile(_ctx.engine))
            tbl.metrics.append(
