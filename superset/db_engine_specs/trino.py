...
# under the License.
# mypy: ignore-errors
"""Trino / PrestoSQL database engine spec."""
...
try:  # pragma: no cover -- optional dep; match Trino HttpError 401 for OAuth2
...
class PrestoBaseEngineSpec(BaseEngineSpec, metaclass=ABCMeta):
    """Shared base for Presto and Trino engine specs."""
    ...
    def get_catalog_names(
        ...
        """Use ``SHOW CATALOGS``. Must wrap in ``text()`` for SA 2.0
        (Engine.execute shim removed)."""
...
class TrinoEngineSpec(PrestoBaseEngineSpec):
    ...
    def handle_cursor(cls, cursor: Cursor, query: Query) -> None:
        """WARNING: direct cursor.execute() blocks; use
        ``execute_with_cursor`` for async handling."""
    ...
    def execute_with_cursor(
        ...
        """Run execute in a thread so handle_cursor can poll for
        the query ID in parallel.

        Trino blocks until complete; the thread lets a stop request cancel via the ID.
        """
        import threading
        import time
        ...
        time.sleep(0.1)
        # Wait for a query ID before handling the cursor; may never appear on error.
    ...
    def update_params_from_encrypted_extra(
        ...
        try:
            ...
            if auth_method == "basic":
                ...
                # Custom auth: must be listed in ALLOWED_EXTRA_AUTHENTICATIONS["trino"].
    ...
    def _expand_columns(cls, col: ResultSetColumnType) -> list[ResultSetColumnType]:
        """Recursively expand ROW columns into dotted-path sub-columns.

        Only named ROW fields are navigable; MAP/ARRAY and anonymous
        ROW fields are skipped.
        """
    ...
    def get_indexes(
        ...
        # Trino dialect raises NoSuchTableError in get_indexes for empty tables.
        try:
