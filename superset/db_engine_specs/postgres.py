...
class PostgresEngineSpec(PostgresBaseEngineSpec):
    ...
    def get_schema_from_engine_params(
        ...
        While Postgres doesn't support connecting directly to a given schema,
        it allows users to specify a "search path" that is used to resolve
        ...
        if search_path := options.get("search_path"):
            ...
            if len(schemas) > 1:
                raise Exception(  # noqa: TRY002
                    ...
                    "means Superset is unable to determine the schema of "
                    "unqualified table names and enforce permissions."
                )
            return schemas[0]
        return None
    ...
    def get_prequeries(
        ...
        This is important for two reasons: in SQL Lab it will allow queries to
        run in the schema selected in the dropdown, resolving unqualified table
        names to the expected schema.  But more importantly, in SQL Lab this is
        used to check if the user has access to any tables with unqualified
        names.
        """
...
WHERE datistemplate = false;
    ...
    def get_datatype(cls, type_code: Any) -> str | None:
        ...
        pg_types = binary_types.copy()
        pg_types.update(string_types)
        if type_code in pg_types:
            return pg_types[type_code].name
        return None
