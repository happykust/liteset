...
uv pip compile pyproject.toml requirements/base.in -o requirements/base.txt --python-version "$PYTHON_TARGET" $ADDITIONAL_ARGS
...
uv pip compile requirements/development.in -c requirements/base.txt -o requirements/development.txt --python-version "$PYTHON_TARGET" $ADDITIONAL_ARGS

# NOTE translation is intended as a "supplemental" set of pins that can be combined with either base or dev as needed
uv pip compile requirements/translations.in -o requirements/translations.txt --python-version "$PYTHON_TARGET" $ADDITIONAL_ARGS
