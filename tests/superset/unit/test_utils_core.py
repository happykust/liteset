...
from superset.utils.column import cast_to_boolean
...
from superset.utils.database import (
    _get_query_source_from_request as get_query_source_from_request,
)
from superset.utils.date import DateColumn, normalize_dttm_col
from superset.viz import _get_stacktrace as get_stacktrace
...
@pytest.mark.skip(
    reason="remove_extra_adhoc_filters is not part of the Litestar port "
    "(no such helper in superset.utils.*); full upstream body and cases "
    "retained below for parity record"
)
...
def test_remove_extra_adhoc_filters(
    ...
    remove_extra_adhoc_filters(original)  # noqa: F821 — helper not ported
    assert expected == original


@pytest.mark.skip(
    reason="is_test() is not part of the Litestar port: there is no "
    "SUPERSET_TESTENV env-gate helper in superset.utils.* (test env is "
    "selected via LITESET_* settings, not a runtime is_test() toggle)"
)
def test_is_test():
    import os
    ...
    assert is_test()  # noqa: F821 — helper not ported
    os.environ["SUPERSET_TESTENV"] = "false"
    assert not is_test()  # noqa: F821 — helper not ported
...
def test_get_datasource_full_name():
    """
    Test the datasource full-name formatting.

    The standalone upstream ``get_datasource_full_name`` helper is not part of
    the Litestar port; the identical ``[db].[catalog].[schema].[table]``
    permission-name formatting now lives on the ``SqlaTable.full_name`` model
    property, so we exercise the exact upstream cases against that property.
    """
    from superset.models.connectors import SqlaTable

    table = SqlaTable(table_name="table", schema="schema", catalog="catalog")
    table.database = "db"
    assert table.full_name == "[db].[catalog].[schema].[table]"

    table = SqlaTable(table_name="table")
    table.database = "db"
    assert table.full_name == "[db].[table]"

    table = SqlaTable(table_name="table", schema="schema")
    table.database = "db"
    assert table.full_name == "[db].[schema].[table]"

    table = SqlaTable(table_name="table", catalog="catalog")
    table.database = "db"
    assert table.full_name == "[db].[catalog].[table]"
...
def test_get_query_source_from_request(
    ...
    if referrer:
        ...
        request_mock.headers = {"referer": referrer}
        token = set_current_request(request_mock)
    else:
        # No bound request -> the port returns None (Celery / CLI path).
...
def test_get_user_agent_custom(monkeypatch) -> None:
    settings = MagicMock()
    settings.user_agent_func = lambda database, source: (
...
@pytest.mark.skip(
    reason="parse_js_uri_path_item is not part of the Litestar port "
    "(no such helper in superset.utils.*); full upstream body retained "
    "below for parity record"
)
def test_parse_js_uri_path_items_eval_undefined():
    assert parse_js_uri_path_item("undefined", eval_undefined=True) is None  # noqa: F821 — helper not ported
    assert parse_js_uri_path_item("null", eval_undefined=True) is None  # noqa: F821 — helper not ported
    assert "undefined" == parse_js_uri_path_item("undefined")  # noqa: F821 — helper not ported
    assert "null" == parse_js_uri_path_item("null")  # noqa: F821 — helper not ported
...
def test_parse_js_uri_path_items_unquote():
    assert "slashed/name" == parse_js_uri_path_item("slashed%2fname")  # noqa: F821 — helper not ported
    assert "slashed%2fname" == parse_js_uri_path_item("slashed%2fname", unquote=False)  # noqa: F821 — helper not ported
...
def test_parse_js_uri_path_items_item_optional():
    assert parse_js_uri_path_item(None) is None  # noqa: F821 — helper not ported
    assert parse_js_uri_path_item("item") is not None  # noqa: F821 — helper not ported


def test_get_stacktrace():
    # In the port, get_stacktrace reads SHOW_STACKTRACE from the pydantic
    # settings passed in, rather than the Flask app config.
    settings_on = MagicMock()
    settings_on.show_stacktrace = True
    try:
        raise Exception("NONONO!")
    except Exception:
        stacktrace = get_stacktrace(settings_on)
        assert "NONONO" in stacktrace

    settings_off = MagicMock()
    settings_off.show_stacktrace = False
    try:
        raise Exception("NONONO!")
    except Exception:
        stacktrace = get_stacktrace(settings_off)
