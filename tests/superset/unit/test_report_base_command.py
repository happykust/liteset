...
# under the License.
"""Port of ``tests/unit_tests/commands/report/base_test.py``.

In the Liteset async port the upstream ``BaseReportScheduleCommand``
class is gone; the frequency check lives as the module-level helper
``superset.commands.report._validate_report_frequency`` which appends a
:class:`ReportScheduleFrequencyNotAllowed` to an ``exceptions`` list
instead of raising directly, and reads the minimum interval from
:class:`SupersetSettings` via ``_settings_config_get`` (not
``flask.current_app.config``).

The behaviour is otherwise identical, so this file re-uses the original
parametrisations and drives the helper through a thin wrapper that
re-raises the collected exception, preserving the original assertions.
"""
...
from superset.commands.report import _validate_report_frequency
from superset.commands.report_exceptions import ReportScheduleFrequencyNotAllowed
...
def dynamic_alert_minimum_interval(**kwargs: Any) -> int:
    return int(timedelta(minutes=10).total_seconds())


def dynamic_report_minimum_interval(**kwargs: Any) -> int:
    return int(timedelta(minutes=5).total_seconds())


def validate_report_frequency(schedule: str, report_type: str) -> None:
    """Drive the Liteset helper the way the upstream method behaved.

    The async port collects validation failures into a list; here we
    re-raise the first collected exception so the original ``pytest.raises``
    / no-raise assertions hold verbatim.
    """
    exceptions: list[Any] = []
    _validate_report_frequency(schedule, report_type, exceptions)
    if exceptions:
        raise exceptions[0]
...
def app_custom_config(
    ...
    """Mock the configured minimum-interval values dynamically per test.

    Upstream patched ``flask.current_app.config``; the Liteset helper reads
    the same keys through ``_settings_config_get(settings, key, default)``,
    so we patch that function to return the per-test overrides.
    """
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            def fake_config_get(_settings: Any, key: str, default: Any) -> Any:
                return config_overrides.get(key, default)

            with patch(
                "superset.commands.report._settings_config_get",
...
def test_validate_report_frequency(report_type: str, schedule: str) -> None:
    ...
    """
    validate_report_frequency(schedule, report_type)
...
def test_validate_report_frequency_minimum_set() -> None:
    ...
    """
    validate_report_frequency("1,5 * * * *", ReportScheduleType.ALERT)
    validate_report_frequency("6,11 * * * *", ReportScheduleType.REPORT)
...
def test_validate_report_frequency_invalid_schedule() -> None:
    ...
    with pytest.raises(ReportScheduleFrequencyNotAllowed):
        validate_report_frequency("1,2 * * * *", ReportScheduleType.ALERT)

    with pytest.raises(ReportScheduleFrequencyNotAllowed):
        validate_report_frequency("1,5 * * * *", ReportScheduleType.REPORT)
...
def test_validate_report_frequency_alert_only(schedule: str) -> None:
    ...
    """
    validate_report_frequency(schedule, ReportScheduleType.REPORT)
...
def test_validate_report_frequency_report_only(schedule: str) -> None:
    ...
    """
    validate_report_frequency(schedule, ReportScheduleType.ALERT)
...
def test_validate_report_frequency_invalid_config(
    ...
    validate_report_frequency(schedule, report_type)
    # ``ReportScheduleType`` is a ``str`` enum whose ``.value`` ("Alert" /
    # "Report") drives the config key (``ALERT_MINIMUM_INTERVAL`` /
    # ``REPORT_MINIMUM_INTERVAL``) logged by the helper.
    config_key = ReportScheduleType(report_type).value
    expected_error_message = (
        f"invalid value for {config_key}_MINIMUM_INTERVAL: 10 minutes"
    )
...
def test_validate_report_frequency_using_callable() -> None:
    ...
    with pytest.raises(ReportScheduleFrequencyNotAllowed):
        validate_report_frequency("1,10 * * * *", ReportScheduleType.ALERT)

    validate_report_frequency("1,11 * * * *", ReportScheduleType.ALERT)

    # Should fail with a 4 minutes interval, and work with 5
    with pytest.raises(ReportScheduleFrequencyNotAllowed):
        validate_report_frequency("1,5 * * * *", ReportScheduleType.REPORT)

    validate_report_frequency("1,6 * * * *", ReportScheduleType.REPORT)
