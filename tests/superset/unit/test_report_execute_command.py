...
# under the License.
"""Port of ``tests/unit_tests/commands/report/execute_test.py``.

Adapted to the Liteset async port:

* ``BaseReportState.__init__`` takes a 4th ``session`` argument (the sync
  Celery ``Session``); the tests pass a ``MagicMock``.
* Dashboard tab URLs are built by ``get_url_path`` against
  ``WEBDRIVER_BASEURL`` and the permalink key comes from the synchronous
  ``BaseReportState._create_dashboard_permalink`` (the async
  ``CreateDashboardPermalinkCommand`` is not used inside the Celery
  context), so the permalink mock targets that method.
* Slack v2 conversion resolves channel ids through
  ``superset.controllers.report._get_slack_channels`` (the port of
  ``get_channels_with_search``), so that is the patched symbol.
* ``HeaderDataType`` lives in ``superset.reports.notifications.base`` and
  ``DashboardPermalinkState`` has no port (``_get_tab_url`` takes a plain
  ``dict``).
"""
...
from superset.commands.report_execute import BaseReportState
from superset.exceptions import UpdateFailedError
...
def _make_state(
    ...
    """Instantiate ``BaseReportState`` with a mocked sync session."""
...
def test_log_data_with_chart(mocker: MockerFixture) -> None:
    ...
    class_instance = _make_state(mock_report_schedule)
...
@patch("superset.commands.report_execute.BaseReportState._create_dashboard_permalink")
def test_get_dashboard_urls_with_multiple_tabs(
    mock_permalink,
    mocker: MockerFixture,
    anchors,
    permalink_side_effect,
    expected_paths,
) -> None:
    mock_report_schedule: ReportSchedule = mocker.Mock(spec=ReportSchedule)
    mock_report_schedule.chart = False
    ...
    class_instance = _make_state(mock_report_schedule)
    class_instance._report_schedule = mock_report_schedule
    mock_permalink.side_effect = permalink_side_effect

    settings = MagicMock()
    settings.feature_flags = {"ALERT_REPORT_TABS": True}
...
def test_get_dashboard_urls_with_exporting_dashboard_only(
    ...
    mock_permalink.return_value = "url1"
...
def test_get_tab_urls(
    ...
    class_instance = _make_state(mock_report_schedule)
    class_instance._report_schedule = mock_report_schedule
    mock_permalink.side_effect = ["uri1", "uri2"]
    tab_anchors = ["1", "2"]

    settings = MagicMock()
    settings.webdriver_baseurl = BASE_URL

    with patch("superset.utils.urls._cached_settings", return_value=settings):
...
def test_get_tab_url(
    ...
    class_instance = _make_state(mock_report_schedule)
    class_instance._report_schedule = mock_report_schedule
    mock_permalink.return_value = "uri"
    # ``DashboardPermalinkState`` has no Liteset port; ``_get_tab_url`` takes
    # a plain mapping of the same shape.
    dashboard_state = {
        "anchor": "1",
        "dataMask": None,
        "activeTabs": None,
...
def test_screenshot_width_calculation(
    ...
    # Initialize BaseReportState (sync session is mocked)
    ...
    # Mock the settings read by ``_get_screenshots``.
    settings = MagicMock()
    settings.alert_reports_executors = {}
    settings.alert_reports_max_custom_screenshot_width = max_width
    settings.webdriver_window = {
    ...
    with (
        ...
        patch(
            "superset.commands.report_execute.get_executor",
            return_value=("executor", "username"),
        ),
        patch.object(BaseReportState, "_find_user", return_value=mocker.MagicMock()),
        patch(
            "superset.utils.screenshots.ChartScreenshot.get_screenshot",
            return_value=b"screenshot bytes",
        ),
        patch(
            "superset.commands.report_execute.ChartScreenshot",
...
def test_update_recipient_to_slack_v2(mocker: MockerFixture):
    ...
    mock_cmmd = _make_state(mock_report_schedule)
