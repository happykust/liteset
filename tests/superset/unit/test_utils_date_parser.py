...
The ``CHART_PLUGINS_EXPERIMENTAL`` feature flag (which gates the
``instant_time_comparison_range`` behaviour) is toggled by monkeypatching
``feature_flag_manager.is_feature_enabled`` -- the established Liteset pattern --
instead of the Flask ``@with_feature_flags`` decorator.
"""
...
@patch("superset.utils.date.parse_human_datetime", mock_parse_human_datetime)
...
@patch("superset.utils.date.parse_human_datetime", mock_parse_human_datetime)
def test_get_since_until_instant_time_comparison_enabled(monkeypatch) -> None:
    monkeypatch.setattr(
        feature_flag_manager,
        "is_feature_enabled",
        lambda feature: feature == "CHART_PLUGINS_EXPERIMENTAL",
    )
...
def test_previous_calendar_quarter() -> None:
...
def test_datetime_eval() -> None:
    ...
    result = datetime_eval(
        "datediff(datetime('2018-01-01T00:00:00'), datetime('2018-01-10T00:00:00'), day)"  # noqa: E501
    )
    assert result == 9

    result = datetime_eval(
        "datediff(datetime('2018-01-01T00:00:00'), datetime('2018-01-10T00:00:00'), year)"  # noqa: E501
    )
    assert result == 0
...
@patch("superset.utils.date.datetime")
