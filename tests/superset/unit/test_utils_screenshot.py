...
def test_get_screenshot(mocker: MockerFixture, screenshot_obj, mock_user):
...
def test_get_from_cache_key(screenshot_obj):
    ...
    assert cache_payload._image == fake_bytes


class TestComputeAndCache:
    def _setup_compute_and_cache(
    ...
    def test_happy_path(self, mocker: MockerFixture, screenshot_obj, mock_user):
        self._setup_compute_and_cache(mocker, screenshot_obj, mock_user)
        screenshot_obj.compute_and_cache(force=False, user=mock_user)
        cache_payload: ScreenshotCachePayloadType = screenshot_obj.cache.get("key")
        assert cache_payload["status"] == "Updated"

    def test_screenshot_error(self, mocker: MockerFixture, screenshot_obj, mock_user):
    ...
    def test_resize_error(self, mocker: MockerFixture, screenshot_obj, mock_user):
        mocks = self._setup_compute_and_cache(mocker, screenshot_obj, mock_user)
    ...
    def test_skips_if_computing(
        ...
        screenshot_obj.compute_and_cache(force=True, user=mock_user)
        get_screenshot.assert_called_once()
        cache_payload: ScreenshotCachePayloadType = screenshot_obj.cache.get("key")
        assert cache_payload["status"] == "Updated"

    def test_skips_if_updated(self, mocker: MockerFixture, screenshot_obj, mock_user):
        mocks = self._setup_compute_and_cache(mocker, screenshot_obj, mock_user)
        ...
        screenshot_obj.compute_and_cache(
            force=False, user=mock_user, window_size=window_size, thumb_size=thumb_size
        )
        get_screenshot = mocks.get("get_screenshot")
        get_screenshot.assert_not_called()
        ...
        # Ensure that it processes when force = True
        screenshot_obj.compute_and_cache(
            force=True, user=mock_user, window_size=window_size, thumb_size=thumb_size
        )
        get_screenshot.assert_called_once()
        cache_payload: ScreenshotCachePayloadType = screenshot_obj.cache.get("key")
        assert cache_payload["image"] != b"initial_value"

    def test_resize(self, mocker: MockerFixture, screenshot_obj, mock_user):
        mocks = self._setup_compute_and_cache(mocker, screenshot_obj, mock_user)
        window_size = thumb_size = (10, 10)
        resize_image: MagicMock = mocks.get("resize_image")
        screenshot_obj.compute_and_cache(
        ...
        screenshot_obj.compute_and_cache(
            force=False, user=mock_user, window_size=(1, 1), thumb_size=thumb_size
        )
