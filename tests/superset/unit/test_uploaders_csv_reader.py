...
# under the License.
"""Flask-free port of ``tests/unit_tests/commands/databases/csv_reader_test.py``.

The upstream test wrapped each payload in a Werkzeug ``FileStorage``.  Liteset's
readers normalise their input via ``_to_stream`` (accepting any ``.read``-able
object / raw bytes), so we feed plain ``io.BytesIO`` / ``io.StringIO`` streams
instead — no Flask/Werkzeug import.
"""
