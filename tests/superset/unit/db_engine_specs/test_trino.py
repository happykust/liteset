...
# under the License.
"""Native Liteset port of the upstream sync TrinoEngineSpec unit tests.

Flask-free: no flask, flask_appbuilder or werkzeug.  Where the original test
patched ``superset.config`` knobs or the WSGI ``current_app`` config, this port
patches the corresponding Liteset settings / lazy import sites; the OAuth2 token
test is rewritten against the port's ``httpx``-based async implementation.
"""
...
# ``trino`` is an optional driver dependency; skip the whole module when the
# package (and its SQLAlchemy dialect) isn't installed.
pytest.importorskip("trino")
pytest.importorskip("trino.sqlalchemy")

from trino.exceptions import (  # noqa: E402
    TrinoExternalError,
    TrinoInternalError,
    TrinoUserError,
)
from trino.sqlalchemy import datatype  # noqa: E402
from trino.sqlalchemy.dialect import TrinoDialect  # noqa: E402
...
from superset.typing import GenericDataType, OAuth2ClientConfig, SQLType
from superset.utils import json
from tests.superset.unit.db_engine_specs.utils import (
    assert_column_spec,
    assert_convert_dttm,
)
...
def test_auth_custom_auth(mocker: MockerFixture) -> None:
    from superset.db_engine_specs.trino import TrinoEngineSpec
    ...
    # The port resolves custom auth via the Liteset settings model
    # (``allowed_extra_authentications``) instead of upstream's
    # ``superset.config.ALLOWED_EXTRA_AUTHENTICATIONS``.
    settings = Mock()
    settings.allowed_extra_authentications = {"trino": {"custom_auth": auth_class}}
...
def test_auth_custom_auth_denied(mocker: MockerFixture) -> None:
    from superset.db_engine_specs.trino import TrinoEngineSpec
    ...
    settings = Mock()
    settings.allowed_extra_authentications = {}
    mocker.patch("superset.config.SupersetSettings", return_value=settings)
...
def test_get_extra_table_metadata(mocker: MockerFixture) -> None:
    ...
    # The port resolves the partition indexes through a synchronous inspector
    # rather than ``Database.get_indexes``; route both to the same mock.
    inspector = mocker.MagicMock()
    inspector.get_indexes = db_mock.get_indexes
    inspector.get_view_names.return_value = []
    db_mock.get_inspector.return_value.__enter__.return_value = inspector
...
def test_execute_with_cursor_in_parallel(mocker: MockerFixture) -> None:
...
@pytest.mark.skip(
    reason="Port gap: execute_with_cursor runs the query on a plain "
    "threading.Thread without the upstream @copy_current_request_context "
    "wrapper (see superset/db_engine_specs/trino.py:714-718), so the current "
    "ContextVar context (set_current_user) is intentionally NOT propagated "
    "into the worker thread. The upstream app-context guarantee has no "
    "counterpart in the async port."
)
def test_execute_with_cursor_app_context(mocker: MockerFixture) -> None:
    """Test that `execute_with_cursor` still contains the current context.

    Upstream (sync) asserted that ``execute`` ran inside the live Flask app
    context with ``g.some_value`` preserved across the threaded cursor
    execution. The Liteset port replaces ``g`` with the current-user
    ContextVar; this body asserts that the user set via ``set_current_user``
    would remain visible inside ``execute``. It is skipped because the port's
    thread does not copy the calling context (no
    ``@copy_current_request_context`` equivalent), so the guarantee does not
    hold by design.
    """
    from superset.db_engine_specs.trino import TrinoEngineSpec
    ...
    def _mock_execute(*args, **kwargs):
        # Upstream: assert has_app_context() and g.some_value == "some_value".
...
def test_get_columns_error(mocker: MockerFixture) -> None:
    ...
    Row = namedtuple("Row", ["Column", "Type"])  # noqa: PYI024
...
def test_adjust_engine_params_fully_qualified() -> None:
    ...
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/system/default"
    ...
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/system/new_schema"
    ...
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/new_catalog/default"
    ...
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/new_catalog/new_schema"
...
def test_adjust_engine_params_catalog_only() -> None:
    ...
    uri = TrinoEngineSpec.adjust_engine_params(url, {})[0]
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/system"
    ...
    assert uri.render_as_string(hide_password=False) == "trino://user:pass@localhost:8080/new_catalog"
...
async def test_get_oauth2_token(
    ...
    The port's ``get_oauth2_token`` is async and uses ``httpx.AsyncClient``
    (the upstream sync ``requests`` call was replaced); the test is adapted to
    the new transport while asserting the same request body and response.
    """
    from superset.db_engine_specs.trino import TrinoEngineSpec
    ...
    async_client_cls = mocker.patch("httpx.AsyncClient", return_value=async_client)

    assert await TrinoEngineSpec.get_oauth2_token(oauth2_config, "code") == token
    ...
    # The upstream sync test asserted ``timeout=30.0`` on ``requests.post``; the
    # port moved the timeout onto the ``httpx.AsyncClient(...)`` constructor, so
    # the same 30.0s coverage is preserved here.
