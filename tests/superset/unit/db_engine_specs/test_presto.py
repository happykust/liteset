...
# under the License.
"""Native Liteset port of the upstream sync PrestoEngineSpec unit tests.
...
@pytest.mark.skip(reason="pyhive (PrestoDialect) driver not installed in this env")
def test_where_latest_partition() -> None:
    """
    Test ``where_latest_partition`` SQL compilation against the Presto dialect.

    Requires ``pyhive.sqlalchemy_presto.PrestoDialect`` which is an optional
    driver dependency not available in the Liteset test environment.
    """
