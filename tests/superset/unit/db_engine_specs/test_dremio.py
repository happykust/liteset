...
# under the License.
"""Native Liteset port of the upstream sync DremioEngineSpec unit tests.
...
def test_get_allows_alias_in_select(mocker: MockerFixture) -> None:
    ...
    assert spec.get_allows_alias_in_select(database) is True

    database.get_extra.return_value = {"version": "24.1.0"}
    assert spec.get_allows_alias_in_select(database) is True

    database.get_extra.return_value = {"version": "24.0.0"}
    assert spec.get_allows_alias_in_select(database) is False
