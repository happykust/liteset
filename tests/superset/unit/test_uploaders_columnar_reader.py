...
Werkzeug ``FileStorage`` is replaced by plain ``io.BytesIO`` streams with a
``.name`` attribute so the reader's ``_yield_files`` can sniff the extension
(zip vs parquet) and ``_to_stream`` can normalise the payload.
"""
...
def test_columnar_reader_wrong_columns_to_read():
    reader = ColumnarReader(
...
def test_columnar_reader_invalid_file():
    ...
    bad = io.BytesIO(b"c1")
    bad.name = "test.parquet"
    with pytest.raises(DatabaseUploadFailed) as ex:
        reader.file_to_dataframe(bad)
    assert str(ex.value) == (
...
def test_columnar_reader_zip():
    ...
    with ZipFile(zip_buffer, "w") as zip_file:
        zip_file.writestr("test1.parquet", file1.read())
        zip_file.writestr("test2.parquet", file2.read())
    zip_buffer.seek(0)
    zip_buffer.name = "test.zip"

    df = reader.file_to_dataframe(zip_buffer)
...
def test_columnar_reader_bad_parquet_in_zip():
    ...
    with pytest.raises(DatabaseUploadFailed) as ex:
        reader.file_to_dataframe(zip_buffer)
    assert str(ex.value) == (
