...
def test_custom_dialect() -> None:
    ...
    The application merges ``SQLGLOT_DIALECTS_EXTENSIONS`` into the global
    ``SQLGLOT_DIALECTS`` map at startup. Here we exercise that same merge
    directly and assert the custom dialect becomes resolvable.
    """
    extensions = {"custom": Dialects.MYSQL}
    try:
        SQLGLOT_DIALECTS.update(extensions)
        assert SQLGLOT_DIALECTS.get("custom") == Dialects.MYSQL
    finally:
        SQLGLOT_DIALECTS.pop("custom", None)
...
def test_extract_tables_from_jinja_sql_disabled(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        ...
        mocker.MagicMock(is_feature_enabled=lambda feature: False),
    )
