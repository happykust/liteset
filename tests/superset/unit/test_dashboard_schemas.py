...
def test_dashboard_copy_body():
    body = msgspec.json.decode(
        b'{"dashboard_title": "Copy", "json_metadata": "{}",'
        b' "duplicate_slices": true, "css": ".my-class {}"}',
...
def test_import_v1_dashboard():
    payload = msgspec.json.decode(
        b'{"dashboard_title": "Imported", "uuid": "abc-123",'
        b' "position": {"ROOT_ID": {"type": "ROOT"}}}',
...
def test_import_v1_dashboard_full():
    body = msgspec.json.decode(
        b'{"uuid": "abc-123", "dashboard_title": "T",'
        b' "certified_by": "Admin", "published": true, "tags": ["tag1"]}',
        type=ImportV1Dashboard,
    )
...
def test_screenshot_body_url_params_tuple_format():
    ...
    # Decoded as (key, value) 2-tuples — the schema enforces the pair shape.
    assert body.url_params == [("key1", "val1"), ("key2", "val2")]


def test_dashboard_json_metadata_stagger():
    meta = msgspec.json.decode(
        b'{"stagger_refresh": true, "stagger_time": 10,'
        b' "filter_bar_orientation": "VERTICAL"}',
