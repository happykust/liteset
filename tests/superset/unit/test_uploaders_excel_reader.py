...
# under the License.
"""Flask-free port of ``tests/unit_tests/commands/databases/excel_reader_test.py``.

Werkzeug ``FileStorage`` is replaced by plain ``io.BytesIO`` streams (with a
``.name`` set so the readers' ``_to_stream`` helper can normalise them).
"""
...
def test_excel_reader_invalid_file():
    ...
    with pytest.raises(DatabaseUploadFailed) as ex:
        excel_reader.file_to_dataframe(io.BytesIO(b"c1"))
    assert str(ex.value) == (
...
def test_excel_reader_metadata_mul_sheets():
    ...
    metadata = excel_reader.file_metadata(buffer)
    assert metadata == {
        "items": [
...
def test_excel_reader_file_metadata_invalid_file():
    ...
    with pytest.raises(DatabaseUploadFailed) as ex:
        excel_reader.file_metadata(io.BytesIO(b"1"))
