...
def test_validate_params_json_with_jinja():
    ...
    The Liteset port raises :class:`JinjaValidationError` directly (the
    legacy code wrapped failures in a Marshmallow ``ValidationError``),
    so the assertions match against that exception and its ``message``.
    """
    ...
    assert "Invalid Jinja2 template" in exc_info.value.message
