...
Verifies that :func:`check_sqlalchemy_uri` blocks filesystem-escaping
analytics-DB drivers (``sqlite``, ``shillelagh``) while allowing safe
ones.
"""
...
def _meta_db_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep the meta-DB feature flag enabled so the shared ``BLOCKLIST``
    global is not mutated with the ``superset$`` regex (which would leak
    across tests). The parametrized cases below never exercise the
    ``superset://`` dialect, matching the original test's intent.
    """
    monkeypatch.setattr(
        analytics_db_safety.feature_flag_manager,
        "is_feature_enabled",
        lambda feature: True,
    )
