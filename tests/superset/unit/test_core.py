...
from superset.typing import AdhocColumn, AdhocMetric
...
def _get_column_mock(params: dict[str, Any]) -> Mock:
    mock = Mock()
...
def get_dataset_mock() -> Mock:
    """Minimal dataset mock with the columns ``get_time_filter_status`` reads."""
    mock = Mock()
    mock.columns = [
        _get_column_mock({"column_name": "ds", "is_dttm": True}),
        _get_column_mock({"column_name": "gender", "is_dttm": False}),
    ]
    return mock
