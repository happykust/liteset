...
@mock.patch("superset.config.SupersetSettings")
@mock.patch("superset.tasks.cache.request.Request")
@mock.patch("superset.tasks.cache.request.urlopen")
def test_fetch_csrf_token(mock_urlopen, mock_request_cls, mock_settings_cls, base_url):
    from superset.tasks.cache import _fetch_csrf_token

    mock_settings = mock.MagicMock()
    mock_settings.webdriver_baseurl = base_url
    ...
    result_headers = _fetch_csrf_token(headers)

    # base is always rstrip("/")-ed, so both slash variants yield the same URL
    expected_url = f"{base_url.rstrip('/')}/api/v1/security/csrf_token/"
