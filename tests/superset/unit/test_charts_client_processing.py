...
from superset.charts.post_processing import apply_client_processing, pivot_df, table
from superset.typing import GenericDataType


# The Liteset ``post_processing`` module's translator is a no-op that
# returns the message verbatim (no gettext dependency); mirror that here so
# the expected markdown matches exactly.
...
def test_pivot_df_no_cols_no_rows_single_metric():
    ...
    # NB: tabulate 0.10.0 renders the large integer verbatim (80679663),
    # not in scientific notation as in upstream's tabulate 0.9.0 expectations.
...
def test_pivot_df_no_cols_no_rows_two_metrics():
    ...
    # NB: tabulate 0.10.0 renders the large integers verbatim (80679663 /
    # 80716959), not in scientific notation as in upstream's tabulate 0.9.0
    # expectations.
...
def test_table():
    ...
    # NB: the formatted value ('80,679,663' via d3NumberFormat) matches upstream;
    # tabulate 0.10.0 right-aligns the numeric-looking string column, whereas the
    # upstream expectation (tabulate 0.9.0) left-aligned it. Same value, only the
    # column alignment differs.
    assert (
...
def test_apply_client_processing_csv_format_empty_string():
    ...
    result = {"queries": [{"result_format": "csv", "data": ""}]}
    form_data = {
        "datasource": "19__table",
        "viz_type": "pivot_table_v2",
        "slice_id": 69,
    ...
    assert apply_client_processing(result, form_data) == {
        "queries": [{"result_format": "csv", "data": ""}]
    }
...
def test_apply_client_processing_csv_format_no_data(data):
    ...
    result = {"queries": [{"result_format": "csv", "data": data}]}
    form_data = {
        "datasource": "19__table",
        "viz_type": "pivot_table_v2",
        "slice_id": 69,
    ...
    assert apply_client_processing(result, form_data) == {
        "queries": [{"result_format": "csv", "data": data}]
    }
...
def test_apply_client_processing_json_format_empty_string():
    ...
    result = {"queries": [{"result_format": "json", "data": ""}]}
    form_data = {
        "datasource": "19__table",
        "viz_type": "pivot_table_v2",
        "slice_id": 69,
    ...
    assert apply_client_processing(result, form_data) == {
        "queries": [{"result_format": "json", "data": ""}]
    }
...
def test_apply_client_processing_json_format_data_is_none():
    ...
    result = {"queries": [{"result_format": "json", "data": None}]}
    form_data = {
        "datasource": "19__table",
        "viz_type": "pivot_table_v2",
        "slice_id": 69,
    ...
    assert apply_client_processing(result, form_data) == {
        "queries": [{"result_format": "json", "data": None}]
    }


def test_apply_client_processing_verbose_map():
