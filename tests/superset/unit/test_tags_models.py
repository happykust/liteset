...
The Liteset port of ``get_tag`` is async (lives in :mod:`superset.tags.core`)
...
def _make_session(existing_tag: Tag | None = None) -> MagicMock:
    """Build a mocked ``AsyncSession`` whose SELECT resolves to *existing_tag*.

    Mirrors the read-then-insert flow of :func:`superset.tags.core.get_tag`:
    ``await session.execute(stmt)`` -> ``.scalars().one_or_none()``.
    ``begin_nested`` is exposed as an async context manager and ``flush`` as
    an awaitable so the create branch can run without a real database.
    """
    session = MagicMock()

    result = MagicMock()
    result.scalars.return_value.one_or_none.return_value = existing_tag
    result.scalars.return_value.one.return_value = existing_tag
...
async def test_get_tag_returns_plain_string_not_markup() -> None:
    ...
    result = await get_tag(tag_name, session, tag_type)
...
async def test_get_tag_with_special_characters() -> None:
    ...
    for tag_name in tag_names:
        session = _make_session(existing_tag=None)
        result = await get_tag(tag_name, session, TagType.custom)
...
async def test_get_tag_strips_whitespace() -> None:
    ...
    for input_name, expected_name in tag_names_with_whitespace:
        session = _make_session(existing_tag=None)
        result = await get_tag(input_name, session, TagType.custom)
...
async def test_get_tag_returns_existing_tag() -> None:
    ...
    session = _make_session(existing_tag=existing_tag)

    result = await get_tag("existing-tag", session, TagType.custom)
...
async def test_get_tag_creates_new_tag() -> None:
    ...
    await get_tag(tag_name, session, tag_type)
