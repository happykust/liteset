...
class InjectMockUserMiddleware(ASGIMiddleware):
    ...
    Replaces the real ``LitesetAuthMiddleware`` so RBAC guards work
...
class InjectUnauthenticatedUserMiddleware(ASGIMiddleware):
    ...
    semantics that match the production ``LitesetAuthMiddleware`` behaviour.
    """
    ...
    async def handle(
        ...
        if scope["type"] in ("http", "websocket"):
            ...
            # LitesetAuthMiddleware when no credentials are present.
