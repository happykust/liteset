...
async def test_get_chart_data_no_query_context(
    ...
    with pytest.raises(LitesetValidationException, match="no query context"):
        await _get_chart_data(
            controller,
            pk=1,
            dao=mock_chart_dao,
...
async def test_get_chart_data_invalid_json(
    ...
    mock_chart_dao.find_by_id = AsyncMock(return_value=chart)
    with pytest.raises(LitesetValidationException, match="invalid query context"):
        await _get_chart_data(
            controller,
            pk=1,
            dao=mock_chart_dao,
...
@patch("liteset.controllers.chart.ChartDataCommand")
